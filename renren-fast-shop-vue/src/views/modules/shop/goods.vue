  ...
  export default {
    data () {
      return {
        ...
        status:["没审核","已审核","审核未通过","已关闭"],    //代表商品状态，数据库为数字，为了显示在这里这样直接定义
        itemCatList:[],       //代表所有商品分类
      }
    },
    components: {
      AddOrUpdate
    ...
    methods: {
      ...
      findItemCatList(){
        this.$http({
          url: this.$http.adornUrl('/shop/itemcat/findItems'),
          method: 'get',
        }).then(({data}) => {
          if (data.code ==0){
            for (let i = 0; i < data.itemCatList.length; i++) {
              let itemCat =data.itemCatList[i];
              //这一步可以看这个item-cat表   表示通过这个表中的ID当做Key 来拿到对应的 Name值 value
      ...
      addOrUpdateHandle (id) {
        this.$router.push(`/shop-goods-add-or-update?id=${id}`)
        console.log("qin " ,id);
      },
      // 删除
      deleteHandle (id) {
